...
data = {
    ...
    'popcorn': [random.choices(popcorn,weights=poocornPer,k=1)[0] for i in range(10000)],
    'snack': [random.choices(snack,weights=snackPer,k=1)[0] for i in range(10000)],
    'drinks': [random.choices(drinks,weights=drinkPer,k=1)[0] for i in range(10000)],
    'times': [random.choices(times,weights=timePer,k=1)[0] for i in range(10000)],
    'payment': [random.choices(payment,weights=paymentPer,k=1)[0] for i in range(10000)]
}


df = pandas.DataFrame(data)
df.to_csv('cgv2.csv', encoding='utf-8-sig',index=False)
